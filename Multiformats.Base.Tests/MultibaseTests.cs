...
public class MultibaseTests
{
    ...
    {
        // prefix 0 - base2 value 99 - invalid chars
    ...
    {
        Assert.Throws<ArgumentNullException>(() => Multibase.Encode(MultibaseEncoding.Base2, []));
    }
    ...
    {
        Assert.Throws<NotSupportedException>(() => Multibase.Encode((MultibaseEncoding)0x2000, [0, 1, 2, 3]));
    }
    ...
    public void TestTryDecoding_GivenValidEncodedInput_Prefixed(string input, MultibaseEncoding encoding)
    {
        ...
        Assert.True(result);
        Assert.Equal(encoding, decodedEncoding);
        Assert.Equal(expected, Encoding.UTF8.GetString(decodedBytes ?? Array.Empty<byte>()));
    }
