...
static void Process(Options options)
{
    using (options.Source)
    {
        ...
        if (!checksum.SequenceEqual(mh.Digest))
        {
            ...
            {
                Console.WriteLine(
                    $"Digest mismatch, got: {Multibase.EncodeRaw(options.Encoding, mh.Digest)}, wanted: {Multibase.EncodeRaw(options.Encoding, checksum)}");
            }
...
static Options ParseOptions(string[] args)
{
    ...
    while (i < args.Length)
    {
        ...
        if (args[i].StartsWith("-a") || args[i].StartsWith("--algorithm"))
        {
            ...
            var code = Multihash.GetCode(algo);
            if (!code.HasValue)
            {
...
static MultibaseEncoding ParseEncoding(string s)
{
